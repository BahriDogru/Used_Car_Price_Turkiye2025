def preprocessing(data_root, save_csv=False, output_data_root=''):
    """"
    Verilen ham datayı modele predicct etmeye hhazır hale getiren fonksiyon.

    Args:
        data_root: ham verinin çekileceği yol (Path)
        save_csv: temizlenmiş verinin csv dosyası olarak kaydedilip kaydedilmeyeceği (default:False)
        output_data_root: Eğer temizlenmiş veri kaydedilecekse, kaydedilecek yol (Path)
    ...
    df = pd.read_csv(data_root, encoding="ISO-8859-9")
    df.columns = ['Title', 'Address', 'City', 'Price(TL)', 'ListingID', 'ListingDate', 'Brand', 'Series', 'Model',
    ...
    ## Dtype düzeltme
    ######################
    print("Dtype düzenlemesi başladı....")
    ...
    print("Dtype düzenlemesi yapıldı....")
    ###########################
    # Edit Variables - Missing Values
    ##########################
    print("Missing Values işlemleri başladı.....")
    ...
    print("Missing Values giderildi.....")
    ##############################
    # Outlier
    ##############################
    print("Outlier işlemleri başladı......")
    ...
    print("Outlier işlemleri yapıldı......")
    ###########################
    # Feature Extraction
    ##########################
    print("Feature Extraction işlemleri başladı.....")
    ...
    print("Feature Extraction işlemleri bitti.....")
    ########################
    # Encode İşlemleri
    ########################
    print("Encode İşlemleri başladı.....")
    ...
    print("Encode İşlemleri bitti.....")
    ###############################
    # Scale İşlemleri
    ###############################
    print("Scale İşlemleri başladı.....")
    ...
    df.drop(['ListingDate','Price(TL)'], axis=1, inplace=True)
    ...
    print("Scale İşlemleri bitti.....")
    if save_csv:
        df_scale.to_csv(output_data_root+"clean_data.csv", index=False)
        print(f"Model {output_data_root}/clean_data.csv olarak kaydedildi")
    return df_scale


ham_veri = "dataset/new_data.csv" # temizlencek veri setinin yolu
df = preprocessing(data_root=ham_veri)